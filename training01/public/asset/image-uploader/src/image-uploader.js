...
(function($) {
    ...
    $.fn.imageUploader = function(options) {
        ...
        let defaults = {
            ...
            extensions: ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.xls', '.xlsx', '.doc', '.docx', '.ppt', '.pptx', '.pdf', '.txt',
                '.JPG', '.JPEG', '.PNG', '.GIF', '.SVG', '.XLS', '.XLSX', '.DOC', '.DOCX', '.PPT', '.PPTX', '.PDF', '.TXT'
            ],
            mimes: [
                'image/jpeg',
                'image/png',
        ...
        plugin.init = function() {
            ...
            plugin.each(function(i, wrapper) {
                ...
                if (plugin.settings.preloaded.length) {
                    ...
                    let upload_folder = '/plugins/image-uploader/src/file-upload/';

                    // Set preloaded images preview
                    for (let i = 0; i < plugin.settings.preloaded.length; i++) {
                        switch (plugin.settings.preloaded[i].mime_type) {
                            case 'application/pdf':
                                url_icon = upload_folder + 'pdf.png';
                                break;
                            case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
                                url_icon = upload_folder + 'word.png';
                                break;
                            case 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
                                url_icon = upload_folder + 'excel.png';
                                break;
                            case 'application/vnd.openxmlformats-officedocument.presentationml.presentation':
                                url_icon = upload_folder + 'powerpoint.png';
                                break;
                            case 'text/plain':
                                url_icon = upload_folder + 'notepad.png';
                                break;
                            default:
                                url_icon = plugin.settings.preloaded[i].url;
                        }

                        var fileNameIndex = plugin.settings.preloaded[i].url.replace(/[\\]/g, '/').lastIndexOf('/') + 1;
                        var filename = plugin.settings.preloaded[i].url.substr(fileNameIndex);
                        filename = filename.slice(2);
                        if (filename.length > 11) {
                            filename = filename.slice(0, 5) + '...' + plugin.settings.preloaded[i].extension;
                        }

                        $uploadedContainer.append(createImg(filename, plugin.settings.preloaded[i].url, plugin.settings.preloaded[i].id, true, plugin.settings.preloaded[i], i, url_icon));
                    }
        ...
        let createImg = function(filename, url, id, preloaded, data, index, url_icon) {

            var html = `<span class="image-name">${filename}</span>`
            ...
            let $container = $('<div>', { class: 'uploaded-image' }),
                ...
                // Create the img tag
                $img = $('<img>', { src: url }).appendTo($container),

                $name = $(html).appendTo($container),
            ...
            if (preloaded) {
                ...
                let $url = $('<input>', {
                    type: 'hidden',
                    name: plugin.settings.preloadedInputName + '[' + index + '][url]',
                    value: data.url
                }).appendTo($container)

                let $file_name = $('<input>', {
                    type: 'hidden',
                    name: plugin.settings.preloadedInputName + '[' + index + '][file_name]',
                    value: data.file_name
                }).appendTo($container)

                let $extension = $('<input>', {
                    type: 'hidden',
                    name: plugin.settings.preloadedInputName + '[' + index + '][extension]',
                    value: data.extension
                }).appendTo($container)

                let $size = $('<input>', {
                    type: 'hidden',
                    name: plugin.settings.preloadedInputName + '[' + index + '][size]',
                    value: data.size
                }).appendTo($container)

                let $type = $('<input>', {
                    type: 'hidden',
                    name: plugin.settings.preloadedInputName + '[' + index + '][mime_type]',
        ...
        let setPreview = function($container, files) {
            ...
            $(files).each(function(i, file) {
                ...
                switch (file.type) {
                    case 'application/pdf':
                        url = upload_folder + 'pdf.png';
                        break;
                    case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
                        url = upload_folder + 'word.png';
                        break;
                    case 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
                        url = upload_folder + 'excel.png';
                        break;
                    case 'application/vnd.openxmlformats-officedocument.presentationml.presentation':
                        url = upload_folder + 'powerpoint.png';
                        break;
                    case 'text/plain':
                        url = upload_folder + 'notepad.png';
                        break;
                    default:
                        url = URL.createObjectURL(file);
                }
                var arr_name = file.name.split('.');
                var extension = arr_name[arr_name.length - 1];

                var filename = '';
                if (file.name.length > 7) {
                    filename = file.name.slice(0, 5) + '...' + extension;
                }
                // Set preview
                $uploadedContainer.append(createImg(filename, url, dataTransfer.items.length - 1), false);
